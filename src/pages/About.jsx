    ...
    <>
      <section className="pt-36 pb-20 bg-slate-950 relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto px-6 md:px-10 relative z-10 text-center">
          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-red-400  font-medium text-sm tracking-widest uppercase mb-4">Our Story</motion.p>
          ...
          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className=" text-slate-400 text-lg leading-relaxed max-w-2xl mx-auto">
      ...
      <section className="py-24 bg-white">
        <div className="max-w-7xl mx-auto px-6 md:px-10">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-center mb-20">
            <div>
              <p className="text-red-600  font-medium text-sm tracking-widest uppercase mb-3">Where We Are</p>
              <h2 className=" font-800 text-4xl text-slate-900 leading-tight mb-4">Currently live in <span className="text-red-600">Lagos, Nigeria</span></h2>
              <p className=" text-slate-500 text-base leading-relaxed">We're currently serving all of Lagos, Nigeria — with plans to expand across West Africa and the continent. If you're outside Lagos, join our waitlist in the app to be notified when we arrive in your city.</p>
            </div>
            <div className="bg-slate-950 rounded-3xl p-8 text-center">
              ...
              <p className="text-white  font-700 text-2xl">Lagos, Nigeria</p>
              <p className="text-slate-400  text-sm mt-1">All areas covered · Same-day booking</p>
              <div className="mt-4 inline-flex items-center gap-2 bg-red-600/10 border border-red-600/20 text-red-400 text-xs  font-medium px-3 py-1.5 rounded-full">
          ...
          <p className="text-red-600  font-medium text-sm tracking-widest uppercase mb-6 text-center">Our Values</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {values.map((v, i) => (
              <motion.div key={v.title} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }}
                ...
                <h3 className=" font-700 text-slate-900 text-base mb-2">{v.title}</h3>
