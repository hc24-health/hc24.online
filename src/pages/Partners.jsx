    ...
    <>
      <section className="pt-36 pb-20 bg-slate-950 relative overflow-hidden">
        ...
        <div className="max-w-3xl mx-auto px-6 md:px-10 relative z-10 text-center">
          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-red-400  font-medium text-sm tracking-widest uppercase mb-4">Our Network</motion.p>
        ...
        <div className="max-w-5xl mx-auto px-6 md:px-10 space-y-16">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-20">
              ...
              <p className=" text-slate-500">Loading partner network...</p>
            </div>
          ) : (
            <>
              <div>
                <div className="flex items-center gap-3 mb-8">
                  ...
                  <h2 className=" font-700 text-2xl text-slate-900">Partner Laboratories</h2>
                </div>
                {labs.length === 0 ? (
                    <p className=" text-slate-500">No partner laboratories found.</p>
                ) : (
                      ...
                      {labs.map((lab, i) => (
                        <motion.div key={lab.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.07 }}
                          ...
                          <div className="flex items-start justify-between gap-2">
                            <div>
                              <p className=" font-700 text-slate-900 text-base">{lab.name}</p>
                              <p className=" text-slate-500 text-sm mt-0.5">{lab.location}</p>
                            </div>
                            <span className="text-xs  font-medium bg-emerald-100 text-emerald-700 px-2 py-1 rounded-full whitespace-nowrap">{lab.cert}</span>
                          </div>
                        </motion.div>
                      ))}
                    </div>
              ...
              <div>
                <div className="flex items-center gap-3 mb-8">
                  ...
                  <h2 className=" font-700 text-2xl text-slate-900">Partner Hospitals</h2>
                </div>
                {hospitals.length === 0 ? (
                    <p className=" text-slate-500">No partner hospitals found.</p>
                ) : (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                      {hospitals.map((h, i) => (
                        <motion.div key={h.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.07 }}
                          className="border border-slate-100 rounded-2xl p-6 bg-slate-50">
                          <p className=" font-700 text-slate-900 text-base">{h.name}</p>
          ...
          <div className="bg-slate-950 rounded-3xl p-10 text-center">
            <h3 className=" font-700 text-white text-2xl mb-2">Interested in partnering with HC24?</h3>
            <p className=" text-slate-400 text-sm mb-6">We're always looking to expand our network with quality labs and hospitals in Lagos and beyond.</p>
            <a href="/contact" className="inline-flex items-center gap-2 bg-red-600 hover:bg-red-700 text-white px-6 py-3 rounded-xl  font-medium text-sm transition-all">
