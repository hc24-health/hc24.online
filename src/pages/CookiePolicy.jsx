      ...
      <div className="pt-28 pb-20">
        <div className="bg-slate-950 py-16 px-6">
          <div className="max-w-3xl mx-auto text-center">
            ...
            <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className=" font-800 text-4xl md:text-5xl text-white mb-4">Cookie Policy</motion.h1>
        ...
        <div className="max-w-3xl mx-auto px-6 md:px-10 mt-14 space-y-10">
          ...
          <motion.section initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            <h2 className=" font-700 text-slate-900 text-lg mb-6">What Cookies We Use</h2>
              ...
              {cookieTypes.map((c, i) => (
                <motion.div key={c.name} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
                  ...
                  <div className="flex items-center gap-3 mb-2">
                    <h3 className=" font-700 text-slate-900 text-sm">{c.name}</h3>
                    <span className={`text-xs  font-medium px-2 py-0.5 rounded-full ${c.required ? "bg-red-50 text-red-600" : "bg-slate-100 text-slate-500"}`}>
