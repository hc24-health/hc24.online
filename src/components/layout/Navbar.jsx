...
export default function Navbar() {
  ...
  return (
    <motion.nav
      ...
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled ? "bg-white/95 backdrop-blur-md shadow-sm border-b border-slate-100" : "bg-transparent"
        }`}
    >
      <div className="max-w-7xl mx-auto px-6 md:px-10 h-20 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-1">
          {navLinks.map((link) => (
            <a
              key={link.label}
              href={link.href}
              className={`px-4 py-2 rounded-full text-sm font-outfit font-medium transition-all duration-200 hover:bg-white/10 ${scrolled ? "text-slate-600 hover:text-slate-900 hover:bg-slate-100" : "text-white/80 hover:text-white"
                }`}
            >
              {link.label}
            </a>
        ...
        <div className="hidden md:flex items-center gap-3">
          <a
            ...
            className={`flex items-center gap-2 text-sm font-outfit font-medium border px-4 py-2 rounded-xl transition-all duration-200 ${scrolled ? "border-slate-300 text-slate-700 hover:border-slate-500" : "border-white/20 text-white hover:border-white/50"
              }`}
          >
            App Store
          </a>
      ...
      {mobileOpen && (
        ...
        >
          ...
          <Link to="/about" className="block px-3 py-2.5 text-slate-700 font-outfit text-sm rounded-lg hover:bg-slate-50" onClick={() => setMobileOpen(false)}>
            About
          </Link>
